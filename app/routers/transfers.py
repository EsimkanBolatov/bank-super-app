...
async def make_transfer(
    ...
    if not sender_account:
        # Если карта не выбрана явно, берем первую доступную
    ...
    if clean_phone:
        ...
        if recipient_user.accounts:
            ...
            raise HTTPException(status_code=400, detail="У получателя нет карт")
    ...
    if recipient_account and sender_account.id == recipient_account.id:
        raise HTTPException(status_code=400, detail="Перевод на ту же карту невозможен. Выберите другую.")
